...
Every push to `master` triggers `.github/workflows/deploy.yml`, which runs `netlify-cli deploy --prod`. No manual deploys needed. Secrets (`NETLIFY_AUTH_TOKEN`, `NETLIFY_SITE_ID`) are stored in the GitHub repo settings under *Settings → Secrets and variables → Actions*.
...
In the `exp-bars` block (still inside `index.html`), copy an existing bar and adjust `--w`:
...
```html
<div class="cert-item">
  <div class="cert-dot gold"></div>   <!-- gold = featured; remove for standard -->
  <div class="cert-body">
...
```js
{
  ...
  date: 'Jun 2026',
  file: 'pdf/your-file.pdf',   // leave empty string '' if no local PDF
  url: 'https://credly.com/...', // leave empty string '' if no external link
  badge: '🏆'
}
```

The gallery rebuilds automatically from the array on every page load — no HTML changes needed.

---

### Add a new work experience entry

Copy this block inside the `<div class="timeline">` in `index.html`:
...
In `index.html`, find the `.hero-stats` block and change the numbers/labels directly:
